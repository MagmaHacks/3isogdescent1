...
function ThreeIsogenySelmerGroupInternal(Isog) 
  ...
  if ratl then
     ...
     // let's stop using FindSubspace(S3, pred) completely
     ...
     n := #B;
     vprintf Selmer,2: "\nBuilding g --> %o\n", Cputime(t0);
     g := [ S3toA(B[i]) : i in [1 .. n] ];
     vprintf Selmer,2: "\ng is built --> %o\n\n", Cputime(t0);
     ng := [ Norm(g[i]) : i in [1 .. n] ];
     m := #bad;
     M := ZeroMatrix(GF(3), n, m);
     for i in [1 .. n] do
      for j in [1 .. m] do
       M[i, j] := GF(3)!Valuation(ng[i], bad[j]);
      end for;
     end for;
     KM := Kernel(M);
     S3 := sub< S3 | [ &+[Eltseq(KM.i)[j]*B[j] : j in [1 .. n]] : i in [1 .. Dimension(KM)] ] >;
  end if;
  ...
  ModCubesCache := AssociativeArray();
...
if IsDefined(ModCubesCache, p) then
    Wp, fieldtoWp := Explode(ModCubesCache[p]);
else
    Wp, fieldtoWp := MakeModCubes(K,pid);
    ModCubesCache[p] := <Wp, fieldtoWp>;
end if;
     ...
     if tauIsInQp then
        ...
        // EEQp := sub< Wp | EEQp, fieldtoWp(-a1*sigma - a3 - 2*tauInQp) >;
     end if;
...
// printf "p=%o: IsSquare=%o sec, descmap=%o sec, trials=%o\n", p, tissquare, tdesc, numtrials;
